...
abstract class PrimitiveArguments<T, V> {
    ...
    final private ArgumentType type;
    ...
    protected PrimitiveArguments(List<KernelArgument> arguments, ArgumentType type, Function<Integer, T[]> arrayConstructor, Supplier<T> arrayListConstructor, Function<T, V> toArray, Function<Integer, V> internalArrayConstructor) {
        this.type = type;
        this.toArray = toArray;
        this.internalArrayConstructor = internalArrayConstructor;
        this.arguments = arguments.stream()
                .filter(argument -> argument.getType().equals(type))
